function [out, LL, lam, log_lam, s ]  = LLSum_GASFacCop_G(theta, data_u , GLweight, group_code, lam_ini)

% This function computes the sum of (negative) log likelihoods of factor copula with GAS recursion
%
...
[Gcdf_ini, Gpdf_ini] = X_marginals([nuinv_z; nuinv_eps; psi_z], GLweight, x_grid, lam_grid);
...
%%% Evaluate log density factor copula with GAS recursion at each time t
for tt = 1:TT
    ...
    [L_temp, N_derivative] = LL_GASFacCop_G([lam(tt,:)'; nuinv_z; nuinv_eps; psi_z], data_u(tt,:)', GLweight, Gcdf, Gpdf, x_grid, dense_lam_grid, group_code, epsi) ;
