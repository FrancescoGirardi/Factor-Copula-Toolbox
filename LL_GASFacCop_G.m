function [LL_t , N_derivative] = LL_GASFacCop_G(theta_t, u_t, GLweight, Gcdf, Gpdf, x_grid, lam_grid, group_code, epsi)

% This function computes the log likelihood of factor copula
...
for i = 1: max(group_code)
    ...
    Ginv_u_t(inx,1)       = X_inverse_cdf(u, theta_group,     Gcdf, x_grid, lam_grid);
    Ginv_u_t_ateps(inx,1) = X_inverse_cdf(u, theta_group_eps, Gcdf, x_grid, lam_grid);
    
    deno_t(inx,1)         = X_pdf_eval(Ginv_u_t(inx,1),       theta_group,     Gpdf, x_grid, lam_grid);
    deno_t_ateps(inx,1)   = X_pdf_eval(Ginv_u_t_ateps(inx,1), theta_group_eps, Gpdf, x_grid, lam_grid);
    
end

numer_t = X_joint_pdf([Ginv_u_t, Ginv_u_t_ateps], theta_t, GLweight, group_code, epsi);
