function out = X_pdf_eval(q, theta, Gpdf, x_grid, lam_grid)

% The vector of (approximate) marginal densities at the vector q associated with factor model 
%
...
if isempty(inx) == 0
    ...
    GLweight = [Wa Wb];
    for i = 1:length(inx)
        x(inx(i)) = GLquad('X_pdf_helper',1e-5,1-1e-5,GLweight,q(inx(i)),theta);
    end
